...
BaseDir <- "/Users/ruhaabmarkas/Desktop/School"
# Load the data from the csv files

getwd()

procrastinate_raw <- Procrastination
...
######## TO DO: Color by HDI Catoegy 
proc <- subset(procrastinate, select = c(Country, DPMean))

# Order the dataframe by DP mean 
proc_order <- proc[order(proc$DPMean, decreasing = TRUE), ]

# Create a frame with the top 15 entries 
proc_top_15 <- head(proc_order, n = 15)

# Plot a bar chart for the top 15 countries by DP mean 
ggplot(data=proc_top_15, aes(x=Country, y=DPMean)) +
...
# Order the dataframe by DP mean 
proc_order_gp <- proc_gp[order(proc_gp$GPMean, decreasing = TRUE), ]

# Create a frame with the top 15 entries 
proc_gp_top_15 <- head(proc_order_gp, n = 15)

# Plot a bar chart for the top 15 countries by DP mean 
ggplot(data=proc_gp_top_15, aes(x=Country, y=GPMean)) +
  geom_bar(stat="identity", fill="steelblue")+
  theme_minimal()

# Plot a scatter plot for age and income
qplot(Age, Income, colour = Gender, data = procrastinate)

# Create a data frame with age, income, and gender variables
age_income <- subset(procrastinate, select = c(Age, Income)
...
ggqqplot(age_income_cor, age_income_cor$Age, ylab = "Age")
